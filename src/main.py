...
class Stitch:
    ...
    def stitching(self):
        ...
        self.stitchThree([2,4,6])
        cv.imwrite("../2-4-6.jpg", s.pano)
        self.stitchThree([4,6,8])
        cv.imwrite("../4-6-8.jpg", s.pano)
        self.stitchThree([6,8,0])
        cv.imwrite("../6-8-0.jpg", s.pano)

    def stitchThree(self, img_list):
        ...
        K = [img_list[2],img_list[0],img_list[1],img_list[0],img_list[2]]
        for i in range(len(input)):
            h,w,_ = input[i].shape
            input[i] = self.cylindricalWarp(input[i],self.K[K[i]])
            input[i] = cv.copyMakeBorder(input[i],100,100,600,600, cv.BORDER_CONSTANT)

        M = self.matcher.feature_matching(input[-1],input[2]) # match 8 to 6
        M1 = self.matcher.feature_matching(input[-2],input[2]) # match 4 to 6

        y,x,_ = input[2].shape

        out1img = cv.warpAffine(input[-1], M, (x,y))
        out2img = cv.warpAffine(input[-2], M1, (x,y))
        out1mask = cv.warpAffine(input[0], M, (x,y))
        out2mask = cv.warpAffine(input[1], M1, (x,y))

        # Use Laplacian_blending to render the panorama
        lpb = self.Laplacian_blending(out1img,input[2],out1mask,3)
